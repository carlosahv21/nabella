<div>
    <div class="table-settings mx-4 my-4">
        <div class="row justify-content-between align-items-center bg-white rounded-3">
            <div class="col-12   col-lg-3 row m-3">
                <select wire:model="drivers" class="form-select" id="drivers">
    ...
    <!-- Modal ViewScheduling-->
    <div class="modal fade" id="viewScheduling" tabindex="-1" aria-labelledby="modal-default" style="display: none;" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{$title_modal}}</h5>
                    <div class="d-flex justify-content-between pt-2">
                        <button type="button" class="btn p-1" wire:click="editEvent({{ $modelId }})">
                ...
                <div class="modal-body">
                    <div class="bg-card p-2 rounded-lg shadow-md">
                        <div class="flex items-center mb-2">
                            <div class="bg-primary w-3 h-3 rounded-full mr-2"></div>
                            <h4>{{ $patient_name }}</h4>
                        </div>
                        <p class="text-muted-foreground">
                            @if(!empty($date) && Carbon\Carbon::hasFormat($date, 'Y-m-d'))
                                {{ Carbon\Carbon::createFromFormat('Y-m-d', $date)->format('l, d \of F Y') }} &bull; {{ \Carbon\Carbon::parse($pick_up_time)->format('H:i') }} &ndash; {{ \Carbon\Carbon::parse($check_in)->format('H:i') }}
                            @endif
                        </p>
                        <div class="flex items-center mt-2">
                            <i class="material-icons notranslate">directions_car</i>
                            <span class="text-muted-foreground mx-2"> {{ ($type_of_trip == 'pick_up') ? 'Pick up' : 'Drop off' }}</span>
                        </div>
                        <div class="flex items-center mt-1">
                            <i class="material-icons notranslate">person</i>
                            <span class="text-muted-foreground mx-2">{{ $driver_name }}</span>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    let calendar = new FullCalendar.Calendar(document.getElementById("calendar"), {
        ...
        eventClick: function(info) {
            Livewire.emit('openViewModal', info.event.id, info.event.title);
        },
